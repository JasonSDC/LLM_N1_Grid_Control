...
## 5. 当前实验结果（`seed=42`，20 场景）

统一外部指标 `evaluate_action_cost`。LLM-RA 为 Claude 选机 + SLP 的实验结果。

### 5.1 全场景（20 组）

| 指标 | AC-OPF (L2) | AC-OPF (L1) | ΔV-only OPF | Rule+SLP | LLM-RA |
|------|-------------|-------------|-------------|----------|--------|
| 意图成功（无 Fallback） | 17 (85%) | 15 (75%) | 8 (40%) | 15 (75%) | 14 (70%) |
| Fallback 成功 | 3 (15%) | 5 (25%) | 12 (60%) | 4 (20%) | 5 (25%) |
| Fallback 失败 | 0 | 0 | 0 | 1 (5%) | 1 (5%) |
| **整体成功率** | **100%** | **100%** | **100%** | **95%** | **95%** |
| 平均延迟 | 306 ms | 321 ms | 339 ms | 1410 ms | 19258 ms |
| 意图成功时平均 Σ\|ΔP\| | 57.2 MW | 49.0 MW | **0.0 MW** | 2.1 MW | **0.0 MW** |
| 意图成功时平均 Σ\|ΔV\| | 0.1098 p.u. | 0.1077 p.u. | 0.1043 p.u. | 0.0370 p.u. | **0.0260 p.u.** |
| 意图成功时平均 **J_eval** | 68.2 | 59.8 | 10.4 | 5.8 | **2.6** |
| 意图成功时平均 n_act | 9.0 | 9.0 | 9.0 | 3.2 | **2.4** |
...
4. **Rule+SLP** 与 LLM 同套物理映射：稀疏、低 J_eval；意图成功率 75%，整体 95%。  
5. **LLM-RA** 意图成功 14/20、整体 95%；意图成功时 Σ\|ΔP\|≈0、J_eval≈2.6、平均 2.4 台机。与 L2-OPF 共同意图成功的 13 个场景上，J_eval 为 2.6 vs 58.9。

### 5.3 主要结论

1. 低成本主要来自 **稀疏电压控制**（意图成功时 Σ\|ΔP\|≈0），LLM-RA 与 Rule+SLP 都能体现。  
2. Check-0/1 后 OPF 系列不重试，失败即 Fallback；Rule 同样不重试；LLM 可带 hint 换机重试。  
